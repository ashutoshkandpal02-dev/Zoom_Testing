...
function LessonDetail() {
  ...
  const totalItems =
    unitLessons.length + unitQuizzes.length + unitAssignments.length;
  const completedLessons = unitLessons.filter(
    lesson => lesson.completed
  ).length;
  const completedQuizzes = unitQuizzes.filter(
    quiz => quiz.status === 'completed'
  ).length;
  const completedAssignments = unitAssignments.filter(
    assignment =>
  ...
  const handleRequestCertificate = () => {
    ...
    toast.success(
      "Certificate request submitted! You will receive an email when it's ready."
    );
  };
  ...
  return (
    <div className="container py-6 max-w-7xl">
      ...
      <div className="p-4 text-center text-gray-500">Lesson view disabled</div>
      ...
      <div className="flex items-center justify-end gap-3 mb-6 opacity-0 action-button">
        ...
        <AlertDialog>
          ...
          <AlertDialogContent className="glass-card">
            <AlertDialogHeader>
              <AlertDialogTitle>
                Request Lesson Completion Certificate
              </AlertDialogTitle>
              <AlertDialogDescription>
                {lessonProgress < 100
