...
function useSpeechSynthesis({
  ...
  const wordBoundariesRef = useRef({ text: '', boundaries: [] });
  ...
  const prepareTextAndBoundaries = useCallback(text => {
    try {
    ...
    (textToSpeak, chunkIndex) => {
      ...
      try {
        ...
        utterance.onerror = event => {
          // "interrupted" (and similar) errors are expected when we cancel
          // or restart speech ourselves – don't treat them as fatal.
          if (event?.error === 'interrupted' || event?.error === 'canceled') {
            console.debug('Speech synthesis interrupted:', event);
            return;
          }
          ...
          if (event.name === 'word') {
            try {
              ...
              if (bestMatch && minDistance < 50) {
                // Use tolerance to avoid false matches
  ...
  const getChunkCharOffset = useCallback(chunkIndex => {
    let offset = 0;
